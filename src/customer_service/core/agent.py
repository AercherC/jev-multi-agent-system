...
class Agent:
    ...
    def __init__(
        ...
        *,
        ...
        jev_router: JevRouter | None = None,   # ← 新增：OpenJev 工具路由复核
    ) -> None:
        self.name = name
        self.llm = llm
    ...
    def run(self, user_message: str, *, history: list[Message] | None = None) -> AgentResult:
        ...
        for step in range(1, self.max_steps + 1):
            ...
            if not resp.tool_calls:
                # 第一步 LLM 没调工具时，用 Jev 复核一次
                if step == 1 and self.jev_router is not None and self.tools is not None:
                    candidate_tools = self.tools.names()
                    jev_pick = self.jev_router.route_tool(user_message, candidate_tools)
                    if jev_pick is not None:
                        # Jev 说该调，但 LLM 没调 → 强制重试一次
                        conv.assistant(resp.content)
                        conv.user(
                            f"请先调用 {jev_pick} 工具查询，不要凭记忆直接回答。"
                        )
                        continue
                # Jev 也说不需要，或没有 Jev → 正常返回
                conv.assistant(resp.content)
                return AgentResult(
