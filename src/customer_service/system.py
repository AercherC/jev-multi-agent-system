...
class MultiAgentSystem:
    ...
    def __init__(
        ...
        *,
        ...
        self.jev_router = JevRouter()
        self._agents: dict[str, Agent] = {
            "FAQ": make_faq_agent(
                llm, max_steps=max_steps, confirmer=confirmer, jev_router=self.jev_router
            ),
            "ORDER": make_order_agent(
                llm, max_steps=max_steps, confirmer=confirmer, jev_router=self.jev_router
            ),
            "COMPLAINT": make_complaint_agent(
                llm, max_steps=max_steps, confirmer=confirmer, jev_router=self.jev_router
            ),
        }
    ...
    def chat(self, user_message: str) -> str:
        ...
        if not cls.intents:
            ...
            # 直接按意图分发，不做外部门控；工具调用与否由各 Agent 内部 Jev 复核
