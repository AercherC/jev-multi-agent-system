...
| | `jev_router.py` | OpenJev 意图判断层：该不该调工具 / 调哪个（choice / noul），异常优雅降级 |
...
3. 每个 Agent 内部 ReAct 循环：LLM 决定调 tool → 执行 → 结果回灌 → 直到给出最终文本（或触达 `max_steps`）。**第一步 LLM 未调工具时，由 `jev_router.route_tool` 复核**：若 Jev 判定该调某工具则回灌提示强制触发一次，解决"自信跳过检索"。
